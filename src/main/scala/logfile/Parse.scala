...
class Parse(
  ...
  def ParseServerLog() = {
    ...
    val userIdPosition =
      configFile.getInt("kugsha.profiles.logfile.userIdPosition")
    val timestampPosition =
      configFile.getInt("kugsha.profiles.logfile.timestampPosition")
    val urlPosition = configFile.getInt("kugsha.profiles.logfile.urlPosition")
    val ignoreList =
      configFile.getStringList("kugsha.profiles.logfile.ignoreList").toList
  ...
  def ParseJsonLog() = {
    ...
    val dateFormat = configFile.getString("kugsha.profiles.logfile.dateFormat")
    val domain =
      configFile.getString("kugsha.crawler.domain").replace("www.", "")

    val product =
      configFile.getStringList("kugsha.profiles.jsonfile.pageMap.product")
    val list =
      configFile.getStringList("kugsha.profiles.jsonfile.pageMap.list")
    val cart =
      configFile.getStringList("kugsha.profiles.jsonfile.pageMap.cart")
    val generic =
      configFile.getStringList("kugsha.profiles.jsonfile.pageMap.generic")
    ...
    Source.fromFile(logPath, "UTF-8").getLines.foreach { line =>
      ...
      if (local.isDefined) {
        ...
        if (url.contains(domain) && eventType.isDefined &&
          ...
          val pageType = (json \ "uri" \ "query" \ "pageType")
            .asOpt[String]
            .getOrElse("notDefined")

          val kind =
            if (product.contains(pageType))
              "product"
          ...
          val cate =
            if (cat.isDefined && cat.get.isEmpty) { Some("notDefined") } else
              cat
          ...
          en =>
            {
              ...
              val sessionLengthDefault =
                configFile.getConfig("kugsha.profiles.session.sessionLength")
              val sessionDurationDefault =
                configFile.getConfig("kugsha.profiles.session.sessionDuration")
              val meanTimePerPageDefault =
                configFile.getConfig("kugsha.profiles.session.meanTimePerPage")

              val sessionLength =
                if (en._1.length <= sessionLengthDefault.getDouble("short"))
              ...
              val sessionDuration =
                if (en._2 <= sessionDurationDefault.getDouble("short"))
